...
$selectData=$obj->selectData("staff_name","tbl_staff"," where status !=0 and staff_login ='$staff_id'");
$maxidRow = mysqli_fetch_assoc($selectData);
$staff_name =$maxidRow['staff_name'];	
...
$insert=$obj->insertData("tbl_credit_note_products",$info_credit_products);
 ...
 $creditnote_activity_log = array(
        "action_done" =>$staff_name."".' updated quantity of product '."".$product_name."".' having old quantity '."".$old_quantity."".' to new quantity '."".$new_qty."".' where id is '."".$pills_id,
        "addedDate" =>$days,
        "addedTime" => $times,
        "status" => 1
    );
