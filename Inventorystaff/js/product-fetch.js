...
function fetch_product_count(){
	$.ajax({
		...
		success:function(result){
			$.when(pageCount_drop_down(end_limit,result)).then(()=>{
...
$('#pagenation_drop').change(function(){
	...
	fetch_product_count()
        ...
        function pageSplit_num(end_limit,total_num){
			end_limit = parseInt(end_limit)
			total_num = parseInt(total_num)
            $('#page_num').empty()
            let num = 0;
			console.log(end_limit,total_num)
...
function pageCount_drop_down(end_limit,total_num){
    ...
    while(total_num>0){
        ...
        if(current_limit == 15){
            ...
            current_limit = parseInt(current_limit) + 25 ;
        }
    }
}
