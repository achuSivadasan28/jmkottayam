...
if($check_security == 1){
	...
	$obj_common->insertData("tbl_patient",$info_insert_data_common);
		...
		if($branch_id == 0){
		$check_user_exist = $obj->selectData("id,unique_id,year,name,phone,address,place,age,gender,whatsApp","tbl_patient","where name='$name' and phone='$number' and status=1");
		$info_insert_data = '';
		if(mysqli_num_rows($check_user_exist)>0){
			...
			$year = $check_user_exist_row['year'];
			$name_w = $check_user_exist_row['name'];
			$phone_w = $check_user_exist_row['phone'];
			$address_w = $check_user_exist_row['address'];
			$place_w = $check_user_exist_row['place'];
			$age_w = $check_user_exist_row['age'];
			$gender_w = $check_user_exist_row['gender'];
			$whatsApp_w = $check_user_exist_row['whatsApp'];
			$check_common_db = $obj_common->selectData("id","tbl_patient","where unique_id='$unique_id' and status!=0");
			if(mysqli_num_rows($check_common_db)){
				
			}else{
				$myString = 'KTM';
				if (strpos($myString, $unique_id) == false) {
    				$inputString = $unique_id;
					$parts = explode('/', $inputString);
					$new_unique_id = $parts[0].'/'.$parts[1].'/KTM/'.$parts[2];
					$unique_No = $parts[2];
					$update_unique_id = array(
						"unique_id" => $new_unique_id
					);
					$obj->updateData("tbl_patient",$update_unique_id,"where id=$p_id");
					$unique_id = $new_unique_id;
	$info_insert_data_common = array(
		"code" => "JMW",
		"year" => $year,
...
We are pleased to inform you that your appointment has been successfully booked at Johnmarians Wellness Hospital. Kindly review the information provided:
...
$template_id = '1207169140950219191';
