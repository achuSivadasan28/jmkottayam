...
if(mysqli_num_rows($select_all_patient_data)>0){
	...
	while($select_all_patient_data_row = mysqli_fetch_array($select_all_patient_data)){
		...
		$select_all_patient_data_date = $obj_branch->selectData("id,treatment_name,file","tbl_assigned_treatment","where added_date='$added_date' and patient_id=$patient_id and status!=0");
