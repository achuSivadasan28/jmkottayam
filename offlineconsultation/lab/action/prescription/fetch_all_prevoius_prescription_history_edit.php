...
if($check_security == 1){
	...
	if(mysqli_num_rows($select_all_prescription_data)>0){
		while($select_all_prescription_data_row = mysqli_fetch_array($select_all_prescription_data)){
			...
			$check_prescriptions_today = $obj->selectData("id,prescriptions_added_date,prescriptions_added_time","tbl_prescriptions","where prescriptions_added_date='$days' and appointment_id=$appointment_id and status=1 and patient_id=$patient_data ORDER BY id DESC");
			if(mysqli_num_rows($check_prescriptions_today)>0){
				...
				while($check_prescriptions_today_row = mysqli_fetch_array($check_prescriptions_today)){
					...
					$select_prescription_details = $obj->selectData("id,prescription_id,medicine_id,medicine_name,quantity,morning_section,noon_section,evening_section,no_of_day,remark,after_food,befor_food","tbl_prescription_medicine_data","where prescription_id=$prescriptions_id and status!=0 ORDER BY id DESC");
			...
			$select_all_prescription_id = $obj->selectData("id,prescriptions_added_date,prescriptions_added_time","tbl_prescriptions","where status!=0 and appointment_id=$appointment_id and patient_id=$patient_data ORDER BY id DESC limit 1");
