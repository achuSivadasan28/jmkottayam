...
if($enc_key_val == $enc_key){
	...
	if($where_details == ''){
	$select_patient_count = $obj->selectData("tbl_appointment.id,tbl_appointment.patient_id,tbl_appointment.doctor_id,tbl_appointment.appointment_date,tbl_patient.name,tbl_patient.phone,tbl_doctor.doctor_name,tbl_patient.place","tbl_appointment inner join tbl_patient on tbl_appointment.patient_id=tbl_patient.id inner join tbl_doctor on tbl_appointment.consulted_by = tbl_doctor.login_id","where tbl_appointment.appointment_status=2 and tbl_appointment.status=1  $where_search_date order by id desc ");
	...
	while($select_patient_count_row = mysqli_fetch_array($select_patient_count)){
		...
		$response_arr[$x]['name'] = $select_patient_count_row['name'];
		$response_arr[$x]['phone'] = $select_patient_count_row['phone'];
		$response_arr[$x]['doctor_name'] = $select_patient_count_row['doctor_name'];
		$response_arr[$x]['place'] = $select_patient_count_row['place'];
		$x++;
	}
	$response_arr[0]['data_exist'] = 1;
	$response_arr[0]['count'] = mysqli_num_rows($select_patient_count);
}else{
