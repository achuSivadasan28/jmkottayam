...
if(isset($_SESSION['doctor_login_id'])){
	...
	if($staff_role == 'doctor'){
		...
		if($check_security == 1){
			...
			$select_appointment = $obj->selectData("branch_id,real_branch_id","tbl_appointment","where id = $appointment_id");
			if(mysqli_num_rows($select_appointment)>0){
				$select_appointment_details = mysqli_fetch_assoc($select_appointment);
				...
				if($branch_id == $real_branch_id){
					$update_status = array(
						"online_conformation_status" => 2,
						"appointment_status" => 2,
						"appointment_end_date" => $days
					);
					$obj->updateData("tbl_appointment",$update_status,"where id=$appointment_id");
					...
					$obj->updateData("tbl_appointment",$update_status,"where id=$appointment_id");
					$obj_branch->updateData("tbl_appointment",$update_status,"where cross_appointment_id = $appointment_id and cross_branch_id = $real_branch_id");
