...
$check_user_exist = $obj->selectData("admin_log_id","tbl_login","where status=1");
//echo $check_user_exist; exit();
if(mysqli_num_rows($check_user_exist)>0){
    $check_user_exist_row = mysqli_fetch_array($check_user_exist);
    $login_id = $check_user_exist_row['admin_log_id'];
//echo $login_id; exit();
  $select_staff_name = $obj->selectData("id,admin_name","tbl_admin","where id=$login_id and status=1");
    //echo $select_staff_name; exit();
	if(mysqli_num_rows($select_staff_name)>0){
        $select_staff_name_row = mysqli_fetch_array($select_staff_name);
        $staff_name = $select_staff_name_row['admin_name'];
    ...
    $obj->updateData("tbl_login",$update_new_pwd,"where admin_log_id=$login_id");
