...
if($check_security == 1){
	...
	$check_doctor_already_added = check_doctor_allready_added($time_slot,$login_id,$url_val,$obj,$date);
	if($check_doctor_already_added == 1){
		$appointment_num = 'same';
	}else{
		$appointment_num = check_total_appointments($date,$login_id,$time_slot,$obj);
	}
	$appointment_data = 0;
