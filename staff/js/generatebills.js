...
if(bill_data == 0 || bill_data == undefined){
	$('.viewPriscription').css('display','none')
	...
	$('.viewPriscription').css('display','flex')
	$('.customer_medicine_details').empty()
    ...
    .then(data => {
					...
					for(let x=0;x<data.length;x++){
						...
						if(data[x]['id'] == medicine_id){
							...
							if(loop_count!=0){
							let template_data = `<div class="customeInputArea customer_medicine_details"> 
<div class="formGroup">
...
<div class="removeBtn">
<button class="inputRemove remove_product"><i class="uil uil-trash"></i></button>
		</div> 
...
//out of stock    
    ...
    $('body').delegate('.Quantity','keyup',function(){
        ...
        let g_total = 0;
		let total_dis_data = 0;
        $('.customer_medicine_details').each(function(){
            let c_price = $(this).find('.Price').val()
            let c_quantity = $(this).find('.Quantity').val()
			let c_discount = $(this).find('.discount').val()
...
//insertion
    $('.aBtn').click(function(e){
        e.preventDefault() 
		valid_c = 0;
		let amt_val = $('.amount_mismatch_value').val()
		if(amt_val == 0){
		$('.aBtn').text('Loading...')
		$('.aBtn').prop('disabled',true)
		$('#common_error_data').text('')
			$('#amt_error_msg').text('')
          $.when(validation_details()).then(function(){
			  let validate_c = valid_c
            if(validate_c == 4){
          ...
          $.when(add_user_details()).then(function(customer_id_data){
			  ...
			  let customer_id = customer_id_json[0]['cid']
                  $.when(save_product_details(customer_id)).then(function(){
					  console.log(loop_count)
					  let product_len = $('.customer_medicine_details').length
					  //console.log("product_len "+product_len)
					 // console.log("loop_count "+loop_count)
					  //$('.shimmer').fadeOut()
					  if(product_len == loop_count){
						  $.when(update_total_product_price(customer_id)).then(function(){
						 $.when(update_payment_type_data(customer_id)).then(function(){
					  if(bill_data != undefined){
					  $.when(update_gen_bill_status(bill_data)).then(function(){
						  
						   $.when(fetch_wp_data(customer_id,bill_data)).then(function(sms_response){
							   $.when(fetch_wp_data1(customer_id)).then(function(sms_response){
							//$.when(send_wp_sms(sms_response)).then(function(){
					setTimeout(function(){
			...
			$('#amt_error_msg').text('*Mismatch In Bill Amount!')
		}
    })

function fetch_wp_data1(customer_id){
		return $.ajax({
			url:"action/fetch_wp_data1.php",
			type:"POST",
...
function send_wp_sms(sms_response){
		let sms_response_json = jQuery.parseJSON(sms_response)
		
		let link = sms_response_json[0]['link']
		let phn = sms_response_json[0]['phn']
		let sms = sms_response_json[0]['sms']
		console.log(link+"&mobile="+phn+"&msg="+sms)
		fetch(link+"&mobile="+phn+"&msg="+sms,{mode:'no-cors'});
	}

	function fetch_wp_data(customer_id,bill_data){
		return $.ajax({
			url:"action/fetch_wp_data.php",
			type:"POST",
			data:{customer_id:customer_id,
				 bill_data:bill_data,
				 }
    ...
    function validation_details(){
      ...
      if(phone_num != ''){
		...
		if(validateDateFormat(created_date)){
			valid_c+=1;
			console.log("No Error")
			$('#date_error').text(' ')
		}else{
			console.log("date Error")
			$('#date_error').text('*Invalid Date Format!')
		}
 ...
 function validateDateFormat(dateString) {
  ...
  // Further validation of the date values
  ...
  // Additional checks like leap year, etc., can be added if needed

  return true; // Valid date format
}

// Example usage
//console.log(validateDateFormat("0023-06-01")); // Output: false
//console.log(validateDateFormat("2023-06-23")); // Output: true
//console.log(validateDateFormat("2023/06/23")); // Output: false (invalid separator)
//console.log(validateDateFormat("2023-13-01")); // Output: false (invalid month)
//console.log(validateDateFormat("2023-06-31")); // Output: false (invalid day)
...
function calculate_total_amt(){
	...
	let total_count = $('.customer_medicine_details').length
	$('.customer_medicine_details').each(function(){
...
$.ajax({
	...
	success:function(payment_result){
		...
		if(payment_result_json.length !=0){
			$('.payment_option').append(`<option value="">Select Payment Option</option>`)
...
$('.viewPriscriptionBtn').click(function(){
	let enc_key = '1efe04280deca02e6e83d619572a3a10'
		$.ajax({
			url:"../offlineconsultation/action/fetch_all_prescription_data_ref.php",
			type:"POST",
			data:{bill_data:bill_data,
				 enc_key:enc_key},
			success:function(result_data){
				...
				for(let x = 0;x<result_data_json.length;x++){
					$('.prescription_details').append(`
						<li>
							<h2>${result_data_json[x]['medicine_name']}</h2>
							<p>${result_data_json[x]['morning_section']}-${result_data_json[x]['noon_section']}-${result_data_json[x]['evening_section']} ${result_data_json[x]['no_of_day']} days</p>
							<span>${result_data_json[x]['food_session']}</span>
						</li>`)
				}
				}else{
		...
		$('.viewPriscriptionPopup').fadeIn();
		$('.shimmer').fadeIn();
	})
$('.closeViewPriscriptionPopup').click(function(){
		$('.viewPriscriptionPopup').fadeOut();
		$('.shimmer').fadeOut();
})

function update_payment_type_data(customer_id){
	let dy_pro_len = $('.dynamic_product_p').length
	let loop_len = 0;
	$('.dynamic_product_p').each(function(){
		let payment_option = $(this).find('.payment_option').val()
		let Recived_amt_in_op = $(this).find('.Recived_amt_in_op').val()
		$.ajax({
			url:"action/update_payment_type.php",
			type:"POST",
			data:{customer_id:customer_id,
				 payment_option:payment_option,
				 Recived_amt_in_op:Recived_amt_in_op
				 },
			success:function(){
				loop_len++
				if(dy_pro_len == loop_len){
					return
				}
			}
		})
...
$('body').delegate('.Recived_amt_in_op','keyup',function(){
	payment_opt_fun()
})

function payment_opt_fun(){
	let total_amt = 0;
	let actual_total_amt = $('#bill_amt').val()
	let payment_op_amt_len = $('.Recived_amt_in_op').length
	let payment_loop_count = 0;
	$('.total_amt_entered').val(0)
	$('.Recived_amt_in_op').each(function(){
...
$('body').on('click','.remove_extra_filed',function(e) {
     e.preventDefault();
