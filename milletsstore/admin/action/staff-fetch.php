...
$fetch_data = $obj1->selectData("id,staff_name,phone,branch,email","tbl_staff","where status=1 $search_where ORDER BY id DESC limit $limit");
if(mysqli_num_rows($fetch_data)>0){
 ...
 while($fetch_data_row = mysqli_fetch_array($fetch_data)){
    ...
    $response_arr[$x]['phone'] = $fetch_data_row['phone'];
	 ...
	 $response_arr[$x]['email'] = $fetch_data_row['email'];
    $x++;
 }
}else{
