...
if($check_security == 1){
	...
	if(mysqli_num_rows($select_todays_appointment)>0){
		...
		while($select_todays_appointment_row = mysqli_fetch_array($select_todays_appointment)){
			...
			$select_patient_data = $obj_branch->selectData("tbl_patient.unique_id,tbl_patient.name,tbl_patient.phone,tbl_patient.address,tbl_patient.place,tbl_patient.age,tbl_patient.gender,tbl_patient.id as patient_id","tbl_patient","where id=$patient_id");
			...
			$select_bmi_history = $obj->selectData("height,weight,appointment_date,blood_pressure","tbl_appointment","where patient_id=$patient_id ORDER BY id DESC");
