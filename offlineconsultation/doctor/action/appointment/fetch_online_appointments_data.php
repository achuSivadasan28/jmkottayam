...
if($check_security == 1){
	...
	$select_todays_appointment = $obj->selectData("tbl_appointment.id,tbl_appointment.doctor_id,tbl_appointment.appointment_status,tbl_appointment.patient_id,tbl_appointment.appointment_number,tbl_appointment.height,tbl_appointment.weight,tbl_appointment.blood_pressure,tbl_appointment.allergies_if_any,tbl_appointment.current_medication,tbl_appointment.present_Illness,tbl_appointment.any_surgeries,tbl_appointment.any_metal_Implantation,tbl_patient.unique_id,tbl_patient.name,tbl_patient.phone,tbl_patient.address,tbl_patient.place,tbl_patient.age,tbl_patient.gender,tbl_patient.id as patient_id,tbl_doctor.doctor_name,tbl_appointment.google_meet,tbl_appointment.schedule_appointment","tbl_appointment inner join tbl_patient on tbl_appointment.patient_id=tbl_patient.id inner join tbl_doctor on tbl_appointment.doctor_id = tbl_doctor.login_id","where  tbl_appointment.status!=0 and tbl_appointment.appointment_status!=2 and tbl_appointment.id = $url_val");
	//echo $select_todays_appointment;exit();
	if(mysqli_num_rows($select_todays_appointment)>0){
		$response_arr[0]['data_status'] = 1;
		$response_arr[0]['current_date'] = $c_days;
		$x = 0;
		$active_id = 0;
		...
		while($select_todays_appointment_row = mysqli_fetch_array($select_todays_appointment)){
			...
			$response_arr[$x]['google_meet'] = $select_todays_appointment_row['google_meet'];
			$response_arr[$x]['schedule_appointment'] = $select_todays_appointment_row['schedule_appointment'];
			...
			$select_bmi_history = $obj->selectData("height,weight,blood_pressure,appointment_taken_date","tbl_appointment","where patient_id=$patient_id");
			if(mysqli_num_rows($select_bmi_history)>0){
				$x1 = 0;
				while($select_bmi_history_row = mysqli_fetch_array($select_bmi_history)){
					if($select_bmi_history_row['height']!= '' and $select_bmi_history_row['weight']!= '' and $select_bmi_history_row['height']!=0 and  $select_bmi_history_row['weight']!=0){
					...
					$appointment_date = $select_bmi_history_row['appointment_taken_date'];
