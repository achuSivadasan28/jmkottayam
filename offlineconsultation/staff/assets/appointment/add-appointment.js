			...
			if(result_data_json[0]['status'] == 1){
				if(result_data_json[0]['data_exist'] == 1){
					...
					for(let x1=0;x1<result_data_json.length;x1++){
						$('.patientNameUl').append(`<li branch-id=${result_data_json[x1]['branch_id']}>${result_data_json[x1]['name']}(${result_data_json[x1]['branch_name']})</li>`)
					}
				}
			}else{
...
function fetch_appointment_fee(){
	$.ajax({
		...
		//console.log(e.target.innerHTML);
			...
			let target = e.target;
			let user_name_all = e.target.innerHTML
			let user_name_arr = user_name_all.split('(')
			thePN.val(user_name_arr[0])
			let branch_id = target.getAttribute("branch-id")
		...
		if(user_name != ''){
			...
			if(result_data_json[0]['status'] == 1){
				if(result_data_json[0]['data_exist'] == 1){
					...
					if(result_data_json[0]['image'] != ""){
						$('#pimage').attr('src',`${result_data_json[0]['image_url']}/offlineconsultation/staff/assets/patientimages/${result_data_json[0]['image']}`)
					}
...
$('.patientNameValue').keyup(function(e){
	...
	let phone_num = $('#number').val();
	let branch_id = $('#branch_id').val();
	if((phone_num.length == 10 || phone_num.length == 12) && user_name != ''){
		//if(user_name != ''){
			$.ajax({
				url:"action/appointment/fetch_user_on_name.php",
				type:"POST",
