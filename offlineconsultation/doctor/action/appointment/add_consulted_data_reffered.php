...
if($check_security == 1){
	...
	$check_cross_appointment = $obj->selectData("id,real_branch_id,branch_id,appointment_date","tbl_appointment","where id=$patient_id");
	if(mysqli_num_rows($check_cross_appointment)){
		$check_cross_appointment_row = mysqli_fetch_array($check_cross_appointment);
		$appointment_date = $check_cross_appointment_row['appointment_date'];
	...
	if(date('Y-m-d',strtotime($appointment_date)) <= date('Y-m-d',strtotime('2023-09-28'))){
...
if(mysqli_num_rows($select_actual_appointment_id)>0){
	...
	$pending_data = 0;
	$obj_branch->updateData("tbl_appointment",$info_update_counsulted,"where cross_appointment_id=$patient_id");
	if(sizeof($food_avoided_plan)!=0){
		for($x1=0;$x1<sizeof($food_avoided_plan);$x1++){
