		...
		if(in_array($hsn_tax,$hsn_number_data)){
			for($y = 0;$y<sizeof($response_arr);$y++){
				if($response_arr[$y]['hsn_number_com'] == $hsn_tax){
					...
					$total_tax_amt += $response_arr[$y]['tax_data'];
					$total_amount += $select_product_details_row['total_price'];
					$total_taxable_value += including_tax_desi($select_product_details_row['tax_in_per'],$select_product_details_row['total_price']);
			...
			$total_amount += $select_product_details_row['total_price'];
			$total_taxable_value += $response_arr[$x]['taxable_value'];
...
$total_tax_amt = $total_amount-$total_taxable_value;
