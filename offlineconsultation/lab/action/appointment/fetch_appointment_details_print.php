...
if($check_security == 1){
	...
	if($patient_id !=0){
		...
		$select_patient_id = $obj->selectData("unique_id,name,phone,address,place,gender,age","tbl_patient","where id=$patient_id and status!=0");
		if(mysqli_num_rows($select_patient_id)>0){
			while($select_patient_id_row = mysqli_fetch_array($select_patient_id)){
				...
				$select_First_visit = $obj->selectData("appointment_date","tbl_appointment","where patient_id='$patient_id' and appointment_status=2 and status!=0 limit 1");
				...
				$select_last_visit = $obj->selectData("appointment_date","tbl_appointment","where patient_id='$patient_id' and appointment_status=2 and status!=0 ORDER BY id DESC limit 1");
				...
				$select_total_num_visit = $obj->selectData("count(id) as id","tbl_appointment","where patient_id='$patient_id' and appointment_status=2 and status!=0");
				...
				if(mysqli_num_rows($select_appointment_details)>0){
					...
					$selectData_prescription = $obj->selectData("id","tbl_prescriptions","where appointment_id=$appointment_id and status!=0");
						...
						if(mysqli_num_rows($selectData_prescription)>0){
							...
							while($selectData_prescription_row = mysqli_fetch_array($selectData_prescription)){
								...
								$selectData_prescription_details = $obj->selectData("medicine_name,quantity,morning_section,noon_section,evening_section,no_of_day,after_food,befor_food","tbl_prescription_medicine_data","where status!=0 and prescription_id=$prescription_id_pk ORDER BY position_data ASC");
					...
					//select current date complaint history
						$select_complaint_history = $obj->selectData("comment","tbl_comments","where patient_id = '$patient_id_unique_id' and added_date ='$appointment_added_date' and status!=0");
					...
					//medical history
				$medical_history = $obj->selectData("id,comment","tbl_medical_history","where patient_id='$patient_id_unique_id' and added_date ='$appointment_added_date' and status!=0");
					       if(mysqli_num_rows($medical_history)>0){
							...
							//surgical history
				$surgical_history = $obj->selectData("id,comment","tbl_surgical_history","where patient_id='$patient_id_unique_id' and added_date ='$appointment_added_date' and status!=0");
						...
						//dite plan to follow
						$select_dite_plan = $obj->selectData("diet","tbl_diet_followed","where appointment_id=$appointment_id and status!=0");
						...
						if(mysqli_num_rows($select_dite_plan)>0){
						  ...
						  //Foods to be avoided
						$select_food_to_be_avoid = $obj->selectData("foods_to_be_avoided","tbl_foods_avoid","where appointment_id=$appointment_id and status!=0");
