...
if($check_security == 1){
	...
	$select_patient_id = $obj->selectData("patient_id,branch_id","tbl_appointment","where id = $appointment_id");
	//echo $select_patient_id;exit();
	if(mysqli_num_rows($select_patient_id) > 0){
		$select_patient_id_rows = mysqli_fetch_assoc($select_patient_id);
		$patient_id = $select_patient_id_rows['patient_id'];
		$branch_id = $select_patient_id_rows['branch_id'];
	...
	$appointment_date = '';
	$appointment_id = 0;
	$check_last_appointmnet_with_fee = $obj_branch->selectData("id,appointment_date","tbl_appointment","where status!=0 and appointment_fee!=0 and appointment_fee_status!=0 and patient_id = '$patient_id' ORDER BY id DESC limit 1");
	...
	$check_phn = $obj_branch->selectData("id,unique_id,name,phone,address,place,age,gender,whatsApp,patient_type","tbl_patient","where id = '$patient_id' and  status!=0");
		//echo $check_phn;exit();
