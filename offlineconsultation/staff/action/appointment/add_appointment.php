			...
			if($total_num_slot!=0){
				if($total_num_slot > $appointment_num){
					if($unique_id == '0'){
						...
						$pk_id = 0;
						$select_pk_id = $obj->selectData("id","tbl_patient","where unique_id='$unique_No_id' and status!=0");
						if(mysqli_num_rows($select_pk_id)>0){
							while($select_pk_id_row = mysqli_fetch_array($select_pk_id)){
								$pk_id = $select_pk_id_row['id'];
							}
						}
					...
					$appointment_status = '';
					$check_appointment_status = $obj->selectData("patient_id,appointment_status","tbl_appointment","where appointment_date='$date' and status != 0 and appointment_status != 2 and patient_id = $patient_id and branch_id = $branch_id and doctor_id = $doctor_data");
					if(mysqli_num_rows($check_appointment_status)){
						$check_appointment_status_rows = mysqli_fetch_assoc($check_appointment_status);
						$appointment_status = $check_appointment_status_rows['appointment_status'];
					}
...
$obj_common->insertData("tbl_appointment_gone_details",$info_appointment_common);
						...
						$response_arr[0]['msg'] = 'The patient is already on the waiting list.';
					}
				}else{
					$response_arr[0]['status'] = 2;
