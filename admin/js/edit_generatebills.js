...
$.ajax({
	...
	success:function(invoice_result){
		...
		let payment = invoice_result_json[0]['payment']
...
<div class="dummyDiv"></div></div>`;
		...
		let payment_len = payment.length;
		let len = 1;
		for(let p = 0;p<payment.length;p++){
			let payment_id = payment[p]['payment_option'];
			let payment_amt = payment[p]['amount'];
			let p_id = payment[p]['id'];
			let payment_option_temp = `<div class="customeInputArea dynamic_product_p" data-id=${p_id}>
						<div class="formGroup">
                           <label for="">Payment Option</label>
							<select class="payment_option" required>`;
							$.when(fetch_all_payment()).then(function(payment_result){
								$.when(append_all_payment_data1(payment_result,payment_id)).then(function(payment_option_temp_dum){
						...
						<div class="formGroup">
                            ...
                            <input type="number" value="${payment_amt}" class="Recived_amt_in_op">
                        </div>
						<div class="removeBtn">
...
function append_all_payment_data1(payment_result,payment_id){
				let payment_option_temp_dum = ``;
				let payment_result_json = jQuery.parseJSON(payment_result);
				console.log(payment_result_json)
				...
				if(payment_result_json.length !=0){
							...
							for(let x=0;x<payment_result_json.length;x++){
								if(payment_id == payment_result_json[x]['id']){
									payment_option_temp_dum += `<option value="${payment_result_json[x]['id']}" selected=true>${payment_result_json[x]['payment_option']}</option>`;
								}else{
...
$('body').delegate('.discount','keyup',function(){
  ...
  $.ajax({
    ...
    $('body').delegate('.Quantity','keyup',function(){
		...
		let exe_count = 0;
		let input_len = $('.customeInputArea').length
...
$('.aBtn1').click(function(e){
	e.preventDefault();
	let amount_mismatch_value = $('.amount_mismatch_value').val()
	if(amount_mismatch_value == 1){
	...
	$.when(update_customer_details(customer_name,phone,place,Tprice,Tdiscount,tamount)).then(function(){
		$.when(update_exist_data()).then(function(){
			$.when(save_product_details(url_val)).then(function(loop_count){
				...
				$.when(update_payment_type_data(url_val)).then(function(){
			//url_val
				setTimeout(function(){
...
function update_payment_type_data(url_val){
	...
	$('.dynamic_product_p').each(function(){
		...
		$.ajax({
			url:"action/update_payment_type_data.php",
			type:"POST",
			data:{customer_id:url_val,
...
function update_customer_details(customer_name,phone,place,Tprice,Tdiscount,tamount){
    ...
    function save_product_details(customer_id){
		let product_len = $('.edit_customer_input_new').length
...
function payment_opt_fun(){
	let total_amt = 0;
	let actual_total_amt = 0;
	let rate_len = $('.Rate').length
	let rate_loop = 0;
	$('.Rate').each(function(){
		if($(this).val() != ''){
		actual_total_amt += parseInt($(this).val())
		}
		rate_loop++
		if(rate_len == rate_loop){
			let payment_op_amt_len = $('.Recived_amt_in_op').length
			console.log("Recived_amt_in_op "+payment_op_amt_len)
