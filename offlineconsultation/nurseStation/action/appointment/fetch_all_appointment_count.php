...
if($check_security == 1){
	...
	$select_todays_appointment = $obj->selectData("count(tbl_appointment.id) as id","tbl_appointment inner join tbl_patient on tbl_appointment.patient_id=tbl_patient.id","where  tbl_appointment.status!=0 $where_search_data $where_date_clause  and tbl_appointment.appointment_status!=2");
