...
if($check_security == 1){
	...
	$patient_data = 0;
	$select_patient_id = $obj_branch->selectData("id","tbl_patient","where unique_id='$patient_id'");
	if(mysqli_num_rows($select_patient_id)>0){
		$select_patient_id_row = mysqli_fetch_array($select_patient_id);
		$patient_data = $select_patient_id_row['id'];
	}
	$select_all_comments = $obj_branch->selectData("DISTINCT added_date","tbl_surgical_history","where patient_id='$patient_id' and status!=0 ORDER BY id DESC");
	...
	for($x = 0;$x<$date_arr_size;$x++){
		...
		$select_all_data = $obj_branch->selectData("id,comment","tbl_surgical_history","where patient_id='$patient_id' and added_date = '$added_date' and status!=0 ORDER BY id DESC");
