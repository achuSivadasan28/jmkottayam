...
if($check_security == 1){
	...
	$prescription_branch_id = $_POST['prescription_branch_id'];
	$branch = $prescription_branch_id;
	...
	$obj_branch->updateData("tbl_prescription_medicine_data",$info_update_p,"where id=$prescription_id");
	
	$select_prescription_id = $obj_branch->selectData("prescription_id","tbl_prescription_medicine_data","where id=$prescription_id");
	if(mysqli_num_rows($select_prescription_id)>0){
		while($select_prescription_id_row = mysqli_fetch_array($select_prescription_id)){
			...
			$check_pres = $obj_branch->selectData("id","tbl_prescription_medicine_data","where prescription_id=$prescription_id_pk and status!=0");
			if(mysqli_num_rows($check_pres) == 0){
				$info_pres_update = array(
					"status" => 0
				);
				$obj_branch->updateData("tbl_prescriptions",$info_pres_update,"where id=$prescription_id_pk");
			}
			$select_patient_id = $obj_branch->selectData("patient_id","tbl_prescriptions","where id=$prescription_id_pk");
			if(mysqli_num_rows($select_patient_id)>0){
				while($select_patient_id_row = mysqli_fetch_array($select_patient_id)){
					...
					$patient_unique_id = $obj_branch->selectData("unique_id","tbl_patient","where id=$patient_id");
