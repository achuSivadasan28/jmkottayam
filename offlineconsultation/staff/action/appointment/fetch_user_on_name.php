...
if($check_security == 1){
	...
	$appointment_date = '';
	$appointment_id = 0;
	$check_last_appointmnet_with_fee = $obj->selectData("id,appointment_date","tbl_appointment","where status!=0 and appointment_fee!=0 and appointment_fee_status!=0 and appointment_fee_status!=0 ORDER BY id DESC limit 1");
	...
	if(mysqli_num_rows($check_phn)>0){
		...
		while($check_phn_row = mysqli_fetch_array($check_phn)){
			...
			$doctor_name = '';
			$check_last_appointmnet_with_fee = $obj->selectData("consulted_by,height,weight","tbl_appointment","where status!=0 and patient_id = $patient_id and appointment_fee_status!=0  ORDER BY id DESC limit 1");	
	if(mysqli_num_rows($check_last_appointmnet_with_fee)>0){
			$check_last_appointmnet_with_fee_row = mysqli_fetch_array($check_last_appointmnet_with_fee);
			...
			require_once '../../../_class_common/query_common.php';
			$obj_common = new query_common();
			$select_image = $obj_common->selectData("image,image_uploadurl","tbl_patient","where unique_id = '$unique_id' limit 1");
			
		    if(mysqli_num_rows($select_image)>0){
			    $select_image_row = mysqli_fetch_assoc($select_image);
				$response_arr[0]['image'] = $select_image_row['image'];
				$response_arr[0]['image_url'] = $select_image_row['image_uploadurl'];
			...
			$select_First_visit = $obj->selectData("first_visit","tbl_appointment","where patient_id='$patient_id' and status!=0 and appointment_fee_status!=0");
			...
			$appointment_date = '';
			$appointment_id = 0;
			$check_last_appointmnet_with_fee = $obj->selectData("id,appointment_date","tbl_appointment","where status!=0 and appointment_fee!=0 and patient_id = $patient_id  and appointment_fee_status!=0 ORDER BY id DESC limit 1");
	...
	//select total visit after payment
			$total_visit_count = 0;
	$select_visit_count = $obj->selectData("count(id) as id","tbl_appointment","where  id>=$appointment_id and patient_id=$patient_id and status!=0 and appointment_fee_status!=0");
	$select_visit_count_row = mysqli_fetch_array($select_visit_count);
	if($select_visit_count_row['id'] != null){
		
		 $selectnumofvist=$obj->selectData("id,num_of_visit","tbl_appointment","where patient_id=$patient_id and id>=$appointment_id and status!=0 and appointment_fee_status!=0");
	...
	$new_format_appointment_date1 = date("d-m-Y", strtotime($new_format_appointment_date));
				$response_arr[0]['total_visit_count'] = $total_visit_count;
	...
	if($total_visit_count<$visit_limit)	{
		...
		$response_arr[0]['appointment_fee'] = 1;
		$response_arr[0]['appointment_fee_nr'] = 500;
	}
		}else{
				$nr = 0;
				$fetch_nr_fee = $obj->selectData("nr","tbl_appointment_fee","where status=1");
				...
				if(mysqli_num_rows($select_nr_limit)>0){
					...
					$check_last_paid_fee = $obj->selectData("appointment_date","tbl_appointment","where appointment_fee!=0 and status!=0 and appointment_fee_status!=0 ORDER BY id DESC limit 1");
	...
	$appointment_date = '';
	$appointment_id = 0;
	$check_last_appointmnet_with_fee = $obj_branch->selectData("id,appointment_date","tbl_appointment","where status!=0 and appointment_fee!=0 and appointment_fee_status!=0 ORDER BY id DESC limit 1");
	if(mysqli_num_rows($check_last_appointmnet_with_fee)>0){
		$check_last_appointmnet_with_fee_row = mysqli_fetch_array($check_last_appointmnet_with_fee);
		$appointment_date = $check_last_appointmnet_with_fee_row['appointment_date'];
		$appointment_id = $check_last_appointmnet_with_fee_row['id'];
	...
	if(mysqli_num_rows($check_phn)>0){
		...
		while($check_phn_row = mysqli_fetch_array($check_phn)){
			...
			$patient_id = $check_phn_row['id'];
			$doctor_name = '';
			$check_last_appointmnet_with_fee = $obj_branch->selectData("consulted_by,height,weight","tbl_appointment","where status!=0 and patient_id = $patient_id and appointment_fee_status!=0 ORDER BY id DESC limit 1");
	if(mysqli_num_rows($check_last_appointmnet_with_fee)>0){
			$check_last_appointmnet_with_fee_row = mysqli_fetch_array($check_last_appointmnet_with_fee);
			$doctor_id = $check_last_appointmnet_with_fee_row['consulted_by'];
			$response_arr[0]['height'] = $check_last_appointmnet_with_fee_row['height'];
			...
			$response_arr[0]['whatsApp'] = $check_phn_row['whatsApp'];
			if($check_phn_row['patient_type'] == 'N' || $check_phn_row['patient_type'] == ''){
			$select_First_visit = $obj_branch->selectData("first_visit","tbl_appointment","where patient_id='$patient_id' and status!=0 and appointment_fee_status!=0");
				//echo $select_First_visit;exit();
				if(mysqli_num_rows($select_First_visit)>0){
					while($select_First_visit_row = mysqli_fetch_array($select_First_visit)){
						$response_arr[0]['first_visit'] = $select_First_visit_row['first_visit'];
			...
			$appointment_date = '';
			$appointment_id = 0;
			$check_last_appointmnet_with_fee = $obj_branch->selectData("id,appointment_date","tbl_appointment","where status!=0 and appointment_fee!=0 and patient_id = $patient_id and appointment_fee_status!=0 ORDER BY id DESC limit 1");
	if(mysqli_num_rows($check_last_appointmnet_with_fee)>0){
			$check_last_appointmnet_with_fee_row = mysqli_fetch_array($check_last_appointmnet_with_fee);
			$appointment_date = $check_last_appointmnet_with_fee_row['appointment_date'];
			$appointment_id = $check_last_appointmnet_with_fee_row['id'];
	...
	//select total visit after payment
			$total_visit_count = 0;
	$select_visit_count = $obj_branch->selectData("count(id) as id","tbl_appointment","where  id>=$appointment_id and patient_id=$patient_id and status!=0 and appointment_fee_status!=0");
	$select_visit_count_row = mysqli_fetch_array($select_visit_count);
	if($select_visit_count_row['id'] != null){
		
		 $selectnumofvist=$obj_branch->selectData("id,num_of_visit","tbl_appointment","where patient_id=$patient_id and id>=$appointment_id and status!=0 and appointment_fee_status!=0");
        $totalvist= mysqli_fetch_assoc($selectnumofvist);
        $total_visit_of_patient = $totalvist['num_of_visit'];
		...
		$response_arr[0]['appointment_fee_msg'] = "Remaining Visit up to ".$new_format_appointment_date1;
			...
			$response_arr[0]['appointment_fee'] = 0;
				...
				if(mysqli_num_rows($select_nr_limit)>0){
					...
					$nr_fee = $select_nr_limit_row['nr'];
					$month_limit_nr = "+".$nr_date_limit." months";
					$check_last_paid_fee = $obj_branch->selectData("appointment_date","tbl_appointment","where appointment_fee!=0 and status!=0 and appointment_fee_status!=0 ORDER BY id DESC limit 1");
					if(mysqli_num_rows($check_last_paid_fee)>0){
						$check_last_paid_fee_row = mysqli_fetch_array($check_last_paid_fee);
						$appointment_taken_date = $check_last_paid_fee_row['appointment_date'];
						$new_format_appointment_date = date("d-m-Y", strtotime($appointment_taken_date));
...
$response_arr[0]['days1'] = $total_visit_count;
