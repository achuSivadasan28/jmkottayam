...
if($check_security == 1){
	$doctor_name = '';
	$branch_name = '';
	$select_doctor_data = $obj->selectData("staff_name,branch_id","tbl_staff","where login_id=$login_id");
	if(mysqli_num_rows($select_doctor_data)>0){
		while($select_doctor_data_row = mysqli_fetch_array($select_doctor_data)){
			$response_arr[0]['proPic'] = '';
			$doctor_name = $select_doctor_data_row['staff_name'];
