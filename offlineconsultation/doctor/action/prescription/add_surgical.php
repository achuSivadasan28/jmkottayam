...
if($check_security == 1){
	$medicalTextarea = $_POST['commentsTextarea'];
	$patient_id = $_POST['patient_id'];
	$data_id = $_POST['data_id'];
	$surgical_branch_id = $_POST['surgical_branch_id'];
	$branch = $surgical_branch_id;
	...
	$obj_branch->insertData("tbl_surgical_history",$info_comment);
		}
	}else{
		if($medicalTextarea == ''){
		$info_comment = array(
		...
		$obj_branch->updateData("tbl_surgical_history",$info_comment,"where id=$data_id");
		}else{
		$info_comment = array(
			"comment" => $medicalTextarea,
		);
