...
$x = 0;
//$id=$_POST['id'];
$customer = $obj1->selectData("id,tax_invoice_year,tax_invoice,tax_compained_val,invoice_no,customer_name,staff_id,phone,created_date,total_price,total_discount,total_amonut,tax_apply,total_tax_amt","tbl_customer","where status!=0 and id=$customer_id  ORDER BY id DESC");
if(mysqli_num_rows($customer)>0){
    while($customer_row = mysqli_fetch_array($customer)){
		...
		$total_quantity = 0;
		$total_price_q = 0;
        $fetch_data = $obj1->selectData("id,customer_id,category_name,product_name,price,no_quantity,gross,total_price,discount,disc","tbl_productdetails","where customer_id=$id");
        if(mysqli_num_rows($fetch_data)>0){
            while($fetch_data_row = mysqli_fetch_array($fetch_data)){
				...
				$total_price_q1 = $fetch_data_row['total_price']*$fetch_data_row['no_quantity'];
				$total_price_q += $total_price_q1;
