...
function fetch_patient_count(){
	$.ajax({
		url:"../offlineconsultation/action/consulted_patient_count.php",
		type:"POST",
...
function fetch_patient_data(){
	$.ajax({
	url:"../offlineconsultation/action/consulted_patient_details.php",
	type:"POST",
	data:{search_val:search_val,
		...
		if(result_json[0]['data_exist'] == 1){
			...
			for(let x1=0;x1<result_json.length;x1++){
				...
				$('.consultAppointmentListTableBody table tbody').append(`<tr>
                                                    ...
                                                    <td data-label="Action">
                                                        <div class="tableBtnArea">
                                                            <a href="" class="tablePrintBtn" data-id=${result_json[x1]['appointment_id']} title="Print"><i class="uil uil-print"></i>
                                                            </a>
                                                            <a href="generate-bill.php?id=${result_json[x1]['appointment_id']} " class="nextBtn">Generate</a>
                                                        </div>
                                                    </td>
                                                </tr>`)
			}
...
function fetch_priscription(appointment_id){
	return $.ajax({
		...
		success:function(prescription_result){
			...
			//console.log(prescription_result_jsaon[0]['data-exist'])
