...
if($check_security == 1){
	...
	if($search_val != ''){
		if($flag){
		     ...
		     $where_date_serach = " and (tbl_patient.name like '%$search_val%' or tbl_patient.phone like '%$search_val%' or tbl_patient.place like '%$search_val%' or tbl_appointment.appointment_number like '%$search_val%' )";
		}
	...
	$select_all_data = $obj->selectData("tbl_appointment.id,tbl_appointment.patient_id,tbl_appointment.doctor_id,tbl_appointment.appointment_date,tbl_appointment.appointment_number,tbl_appointment.branch_id,tbl_doctor.doctor_name,tbl_appointment.status as appointmentstatus,tbl_appointment.appointment_status,tbl_appointment.appointment_fee as appointment_fee,tbl_branch_url.short","tbl_appointment inner join tbl_doctor on tbl_appointment.reffer_status = tbl_doctor.login_id inner join tbl_branch_url on tbl_branch_url.branch_id = tbl_appointment.branch_id ","where tbl_appointment.status!=2 and tbl_appointment.cross_branch_status!=1 and tbl_appointment.reffer_status != 0 and reffered_date = '$days_re'$where_date_clause $where_date_serach  ORDER BY tbl_appointment.id desc");
	...
	if(mysqli_num_rows($select_all_data)>0){
		...
		while($select_all_data_row = mysqli_fetch_array($select_all_data)){
			...
			$response_arr[$x]['branch_id'] = $select_all_data_row['short'];
