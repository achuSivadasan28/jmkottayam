...
$select_patient_list_offline = $obj->selectData("count(id) as id","tbl_appointment","where status!=0 and appointment_taken_type='Offline' and cross_branch_status=0 $where_date_clause");
		...
		$new_patient_count = 0;
		$select_patient_list_new_offline = $obj->selectData("id,patient_id","tbl_appointment","where appointment_taken_type='Offline' and status!=0 and old_patient!=1 and cross_branch_status=0 and appointment_fee=500 and real_branch_id = branch_id $where_date_clause");
		if(mysqli_num_rows($select_patient_list_new_offline)>0){
			while($select_patient_list_new_offline_row = mysqli_fetch_array($select_patient_list_new_offline)){
				...
				$check_patient_id = $obj->selectData("count(id) as id","tbl_appointment","where patient_id=$patient_id and appointment_taken_type='Offline' and status!=0 and id<=$appointment_id and branch_id = real_branch_id");
