...
if($check_security == 1){
	...
	if($search_val == ''){
	$select_todays_appointment = $obj->selectData("tbl_appointment.id as app_id,tbl_appointment.appointment_number,tbl_appointment.present_Illness,tbl_appointment.appointment_date,tbl_appointment.appointment_status,tbl_appointment.doctor_id,tbl_appointment.branch_id,tbl_appointment.patient_id,tbl_doctor.doctor_name","tbl_appointment inner join tbl_doctor on tbl_doctor.login_id=tbl_appointment.doctor_id","where tbl_appointment.status!=0 $where_search_data $where_date_clause and tbl_appointment.appointment_status=2 ORDER by tbl_appointment.id DESC limit $limit_range");
	if(mysqli_num_rows($select_todays_appointment)>0){
		...
		while($select_todays_appointment_row = mysqli_fetch_array($select_todays_appointment)){
			...
			$select_patient_data = $obj_branch->selectData("tbl_patient.unique_id,tbl_patient.name,tbl_patient.phone,tbl_patient.place","tbl_patient","where id=$patient_id $where_search_data");
			if(mysqli_num_rows($select_patient_data)>0){
				$doctor_id = $select_todays_appointment_row[''];
		...
		$select_todays_appointment_patient = $obj_common->selectData("id,logind_id,branch_id,unique_id","tbl_patient","where status!=0 $where_search_data");
		if(mysqli_num_rows($select_todays_appointment_patient)>0){
			...
			while($select_todays_appointment_patient_row = mysqli_fetch_array($select_todays_appointment_patient)){
				...
				$branch = $branch_id;
				$obj_branch = new query_branch();
				$select_patient_data_p = $obj_branch->selectData("tbl_patient.id","tbl_patient","where tbl_patient.status!=0 and tbl_patient.unique_id='$patient_unique_id'");
				
				if(mysqli_num_rows($select_patient_data_p)>0){
					while($select_patient_data_p_row = mysqli_fetch_array($select_patient_data_p)){
				$patient_id = $select_patient_data_p_row['id'];
						$select_todays_appointment = $obj->selectData("tbl_appointment.id as app_id,tbl_appointment.appointment_number,tbl_appointment.present_Illness,tbl_appointment.appointment_date,tbl_patient.unique_id,tbl_patient.name,tbl_patient.phone,tbl_patient.place,tbl_appointment.appointment_status,tbl_appointment.doctor_id,tbl_appointment.branch_id,tbl_appointment.patient_id,tbl_doctor.doctor_name","tbl_appointment inner join tbl_patient on tbl_appointment.patient_id=tbl_patient.id inner join tbl_doctor on tbl_doctor.login_id=tbl_appointment.doctor_id","where tbl_appointment.status!=0 and tbl_appointment.patient_id=$patient_id  $where_date_clause and tbl_appointment.appointment_status=2 ORDER by tbl_appointment.id DESC limit $limit_range");
