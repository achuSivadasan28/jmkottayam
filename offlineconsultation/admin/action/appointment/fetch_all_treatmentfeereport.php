...
$date_where_clause = " and tbl_appointment_modeinvoice.addedDate='$startDate_current'";
	...
	$select_treatment_data = $obj->selectData("tbl_treatment_invoice.invoice_num_com,tbl_treatment_invoice.id,tbl_treatment_invoice.patient_id,tbl_treatment_invoice.added_date,tbl_treatment_invoice.total_amt,appointment_id","tbl_treatment_invoice","where status != 0  and total_amt != 0 $treatment_date_clause limit $limit");
	...
	if(mysqli_num_rows($select_treatment_data)){
		...
		while($select_treatment_data_rows = mysqli_fetch_assoc($select_treatment_data)){
			...
			$appointment_id = $select_treatment_data_rows['appointment_id'];
			$select_branch_id = $obj->selectData("branch_id","tbl_appointment","where id = $appointment_id");
			//echo $select_branch_id;exit();
			$select_branch_id_rows = mysqli_fetch_assoc($select_branch_id);
			$branch_id = $select_branch_id_rows['branch_id'];
			...
			$select_patient_details = $obj_common->selectData("name,phone","tbl_patient","where status != 0 and patient_pk = $patient_id and branch_id = $branch_id");
			//echo $select_patient_details;exit();
