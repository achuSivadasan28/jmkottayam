...
$('body').delegate('.updateData','click',function(e){
	...
	$.when(fetch_all_lab_details(url_val)).then(function(){
	$.when(fetch_all_die_details()).then(function(){
		$.when(fetch_all_foods_to_be_avoided_details()).then(function(){
	//food_avoided_plan
    //diet_plan
...
fetch("https://pala.jmwell.in/api/fetchAll_medicine.php?api="+api)
...
$.ajax({
	...
	success:function(result_data){
		...
		for(let x=0;x<result_data_json.length;x++){
			if(result_data_json[x]['check_status'] == 0){
			$('.multipleSelect ul').append(`<li data-id=${result_data_json[x]['id']}>${result_data_json[x]['test_name']} (${result_data_json[x]['mrp']})</li>`)
			}else{
				$('.multipleSelect ul').append(`<li data-id=${result_data_json[x]['id']} class="dropDownPopuplistMenuActive">${result_data_json[x]['test_name']} (${result_data_json[x]['mrp']})</li>`)
			}
		}
		}
	}
...
$.ajax({
	...
	success:function(result_data){
		...
		if(result_data_json.length !=0){
		for(let x=0;x<result_data_json.length;x++){
			$('.formTagArea').append(`<div class="formTagBox">
	<p class="">${result_data_json[x]['test_name']} (${result_data_json[x]['test_price']})</p>
	<div class="closeFormTagBox" data-id = "${result_data_json[x]['id']}">
