...
$fetch_patient = $obj_branch->selectData("id,name,phone,age,place","tbl_patient","where id='$url_val'");
...
$select_bmi_details = $obj_branch->selectData("weight,height,diet_no_of_days,dite_remark","tbl_appointment","where patient_id=$url_val order by id desc limit 1");
...
$total_number_visit=$total_count['id'];
	 ...
	 $select_todays_appointment = $obj_branch->selectData("tbl_appointment.id,tbl_appointment.appointment_status,tbl_appointment.branch_id,tbl_appointment.consulted_by,tbl_appointment.doctor_id,tbl_appointment.appointment_date,tbl_appointment.patient_id,tbl_appointment.appointment_number,tbl_appointment.height,tbl_appointment.weight,tbl_appointment.blood_pressure,tbl_appointment.allergies_if_any,tbl_appointment.current_medication,tbl_appointment.present_Illness,tbl_appointment.any_surgeries,tbl_appointment.any_metal_Implantation,tbl_patient.unique_id,tbl_patient.name,tbl_patient.phone,tbl_patient.address,tbl_patient.place,tbl_patient.age,tbl_patient.gender,tbl_patient.id as patient_id,tbl_appointment.status","tbl_appointment inner join tbl_patient on tbl_appointment.patient_id=tbl_patient.id","where tbl_appointment.patient_id=$url_val");
...
$doctor_name=$doc['doctor_name'];
			...
			$response_arr[$x]['status'] = $select_todays_appointment_row['status'];
			$select_appointment_details = $obj_branch->selectData("weight,height,diet_no_of_days,dite_remark","tbl_appointment","where id=$appointment_id");
				...
				if(mysqli_num_rows($select_appointment_details)>0){
					...
					if($height != '' && $weight != '' and $height!=0 and  $weight!=0 and $height!=00 and  $weight!=00){
					$height_in_m = $height/100;
					$height_in_m *= $height_in_m;
					$BMI = round($weight/$height_in_m,2);
