...
$fetch_data = $obj1->selectData("tbl_product.id as id,tbl_product.product_name,tbl_medicine_details.id as med_id,tbl_product.category_id,tbl_product.price,tbl_product.discount,tbl_product.quantity","tbl_product inner join  tbl_medicine_details on tbl_product.id = tbl_medicine_details.product_id inner join tbl_category on tbl_category.id = tbl_product.category_id","where tbl_product.status!=0  and tbl_medicine_details.status != 0 and tbl_medicine_details.quantity > 0 and tbl_category.status != 0 $search_where ORDER BY product_name ASC $where_limit");
//echo $fetch_data;exit();
