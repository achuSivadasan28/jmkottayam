...
$customer = $obj1->selectData("id,invoice_no,customer_name,staff_id,phone,created_date,total_price,total_discount,total_amonut","tbl_customer","where status!=0 $search_where  $where_search_date ORDER BY id DESC limit $limit");
if(mysqli_num_rows($customer)>0){
    while($customer_row = mysqli_fetch_array($customer)){
        ...
        $cat_arr = [];
		$total_price = 0;
        $fetch_data = $obj1->selectData("id,customer_id,category_name,product_name,no_quantity,total_price","tbl_productdetails","where customer_id=$id ORDER BY id DESC");
		...
		$response_arr[$x]['total_price'] = round($total_price,2);
