...
<div class="consultationsCardMain">
					...
					<div class="consultationsCardDetails">	
						...
						<div class="cardRight">
						<button class="prescriptionBtn" data-id=${result_data_json[x]['id']} >View Prescription</button>
						<button class="moreDetailsButton" p-id=${result_data_json[x]['id']} >More details</button>	
					</div>
</div>
...
$('.reportsPopup').css('display','block')
	...
	$.when(fetch_labreports(patient_id)).then(function(){
...
$('body').delegate('.prescriptionBtn','click',function(e){
	...
	//alert(appointment_id)
	$.when(fetch_priscription(appointment_id)).then(function(){
	print()
	})
	})
...
$.when(fetch_moredetails(appointment_id)).then(function(){
					
	})
})
	
function fetch_moredetails(appointment_id){
return $.ajax({
		...
		success:function(prescription_result){
			...
			if(prescription_result_jsaon[0]['data-exist'] == 1){
				...
				$('#all_remark').append(`<span>${prescription_result_jsaon[0]['main_remark']}</span>`)
...
function fetch_priscription(appointment_id){
	return $.ajax({
		url:"../action/patient_priscription.php",
		type:"POST",
		data:{appointment_id:appointment_id,
...
function fetch_labreports(patient_id){
$.ajax({
