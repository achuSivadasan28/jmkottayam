...
if($check_security == 1){
	...
	if(mysqli_num_rows($select_all_prescription_data)>0){
		while($select_all_prescription_data_row = mysqli_fetch_array($select_all_prescription_data)){
			...
			$select_all_prescription_id = $obj_branch->selectData("id,prescriptions_added_date,prescriptions_added_time","tbl_prescriptions","where status!=0 and patient_id=$patient_data ORDER BY id DESC");
