...
$fileName = "Johnmarians_bill_trsr" . date('Y-m-d') . ".xls"; 
...
$response_arr = array();
$x=0;
$fetch_data = $obj1->selectData("tbl_product.id,tbl_product.product_name,tbl_product.category_id,tbl_medicine_details.no_of_pills,tbl_medicine_details.quantity","tbl_product inner join tbl_medicine_details on tbl_product.id=tbl_medicine_details.product_id","where tbl_product.status!=0 and tbl_medicine_details.status != 0 ORDER BY id DESC");
if(mysqli_num_rows($fetch_data)>0){
 ...
 while($fetch_data_row = mysqli_fetch_array($fetch_data)){
    ...
    $product_quantity =0;
    $product_id=$fetch_data_row['id'];
    $p_details=$obj1->selectData("no_quantity,product_id","tbl_productdetails inner join tbl_customer on tbl_productdetails.customer_id = tbl_customer.id","where product_id=$product_id and tbl_customer.status !=0 and tbl_productdetails.status != 0 ");
