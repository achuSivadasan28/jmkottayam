...
let url_val_split = url.split("=");
let url_val = url_val_split[1];
//$('.startMeeting').attr('appointmet-id',url_val)
let fun_exe = 0;
...
function fetch_all_todays_appointments(){
	...
	$.ajax({
		url:"action/appointment/fetch_online_appointments_data.php",
		type:"POST",
		...
		success:function(result_data){
			...
			if(result_data_json[0]['data_status'] !=0){
				$('.next_appointmet').attr('min',result_data_json[0]['current_date'])
			if(result_data_json.length !=0){
			...
			for(let x=0;x<result_data_json.length;x++){
				si_no++
				let active = "";
				let google_meet = result_data_json[x]['google_meet']
				//commentsPopup
					...
					$('.currentConsultingheadPrintBtn').attr('data-id',result_data_json[x]['id'])
						fetch_all_prescription_data(result_data_json[x]['unique_id'],5)
					    fetch_all_lab_reports(result_data_json[x]['patient_id'],5)
						fetch_all_prescription_data_history(result_data_json[x]['unique_id'],5)
						fetch_allcomments(result_data_json[x]['unique_id'],5)
						fetch_allmedical(result_data_json[x]['unique_id'],5)
						fetch_allsurgical(result_data_json[x]['unique_id'],5)
						fetch_all_previous(result_data_json[x]['patient_id'],5)
				        fetch_all_remarks_added(result_data_json[x]['patient_id'],5)
					    fetch_all_prev_dite_plan(result_data_json[x]['patient_id'],5)
				        $('.dite_to_follow_btn').attr('branch_id',5)
					    $('.food_to_avoid_btn').attr('branch_id',5)
				$('.patient_history').attr('href','patients-profile.php?patient_id='+result_data_json[x]['patient_id']+'/'+result_data_json[x]['branch']);
					patient_id_one = result_data_json[x]['patient_id'];
					...
					let bmi_si_no = 0;
						...
						let bmi_weight_cat = '';
						console.log("BMI")
						console.log(result_data_json[x]['BMI'])
						for(let bmi_val = 0;bmi_val<result_data_json[x]['BMI'].length;bmi_val++){
							if(result_data_json[x]['BMI'][bmi_val]['BMIVal'] != ''){
						...
						$('.currentConsultingKeepWaitBtn').css('display','flex')
					if(result_data_json[x]['schedule_appointment'] == 1){
					setTimeout(function(){
						$('.startMeeting').attr('href',result_data_json[x]['google_meet'])
						$('.startMeeting').css('display','flex')
						//window.open(result_data_json[x]['google_meet'], '_blank'); 
					},2000)
					}
				...
				$('.allprev_appointments').append(`<tr class="${active}">
                                                    ...
                                                    <td data-label="Action">
                                                        <div class="tableBtnArea">
                                                            <div class="keepWaitBtn" data-id=${result_data_json[x]['id']}>Reschedule</div>
			...
			$.ajax({
				...
				success:function(appointment_result){
					...
					if(appointment_result_json[x]['present_Illness'] != ''){
						...
						fetch_all_prescription_data(appointment_result_json[x]['unique_id'],5)
						fetch_all_prescription_data_history(appointment_result_json[x]['unique_id'],5)
						fetch_allcomments(appointment_result_json[x]['unique_id'],5)
						fetch_allmedical(appointment_result_json[x]['unique_id'],5)
						fetch_allsurgical(appointment_result_json[x]['unique_id'],5)
...
$('body').delegate('.consultedBtn','click',function(e){
	$('.consultationAlert').fadeIn();
	$('.shimmer').fadeIn();
	patient_id = $(this).attr('data-id')
	button_test = $('.active_btn').text().trim()
});
$('.closeconsultationAlert').click(function(){
	$('.consultationAlert').fadeOut();
	$('.shimmer').fadeOut();
});

$('.confirmconsultationAlert').click(function(){
		$('.consultationAlert').fadeOut();
		$('.shimmer').fadeOut();
	if(button_test == 'Done'){
		$('.currentConsulting_patient').css('display','none')
	...
	if(prev_btn_text == 'Update'){
		...
		$.ajax({
			...
			success:function(P_result_data){
				...
				fetch_all_prescription_data(current_patient_uniqueid,5)
				fetch_all_prescription_data_history(current_patient_uniqueid,5)
				}else{
...
<i class="uil uil-trash"></i>
					...
					tamplate += `<tr>
												...
												<td>${all_prescription_json[x1]['evening_section']}</td>
												<td>${all_prescription_json[x1]['time']}</td>
												<td><div class="prescriptionHistoryBtnArea">${edit_btn }${delete_btn}</div></td>
											</tr>`
...
<input type="hidden" class="position" value=${position_data}>
										...
										<div class="formGroup formGroup2" style="">
										...
										<div class="formGroup4">
											<div class="checkBoxArea">
												<input type="radio" class="morning_data" id="afterFood${addmore_c}" name="foodMed2${addmore_c}">
												<label for="afterFood${addmore_c}">After Food</label>
											</div>
											<div class="checkBoxArea">
												<input type="radio" class="noon_data" id="beforeFood${addmore_c}" name="foodMed2${addmore_c}">
									...
									<div class="labTestHistoryReport">`
										for(let y=0;y<date_report.length;y++){
											...
											if(date_report[y]['lab_report_file'] != ''){
												console.log(date_report[y]['upload_from'])
												if(date_report[y]['upload_from'] == 1){
													file_url = 'https://johnmariansconsultation.com/fileupload/';
												}else{
													file_url = '../lab/assets/fileupload/';
												}
											 	file_upload = `<a href="${file_url}${date_report[y]['lab_report_file']}" class="labTestHistoryReportFileBtn">
																	<i class="uil uil-eye"></i>
																</a>`
											}else{
												file_upload = ``;
...
$('#save_lab_report_data').click(function(){
	report_arr = [];
	if($('.tagName').length !=0){
		$(this).text('Loading...')
		let that = $(this)
	$.when(fetch_all_report()).then(function(){
		$.ajax({
			url:"action/appointment/add_lab_reports.php",
			type:"POST",
			data:{report_arr:report_arr,
				 url_val:url_val,
				 },
			success:function(report_result){
				that.text('Success')
				that.css('background-color','blue')
				$('.dropDownInputText').text(' ')
				$('.formTagArea').empty()
				setTimeout(function(){
					fetch_all_previous(patient_id_one)
				},1000)
...
function fetch_all_report(){
	let tag_val = 0;
	let tag_len = $('.tagName').length
	console.log(tag_len)
	$('.tagName').each(function(){
		report_arr.push($(this).attr('data_id'))
		tag_val++
		if(tag_val == tag_len){
			return
		}
...
$('.scheduleNextBtn').click(function(){
	
	$('.datePopup').fadeIn();
	$('.shimmer').fadeIn();
})
$('.closeDatePopup').click(function(){
	$('.datePopup').fadeOut();
	$('.shimmer').fadeOut();
})

$('.submitDatePopup').click(function(e){
	e.preventDefault()
	$(this).text('Loading...')
	let next_date = $('.next_appointmet').val()
	let next_time = $('.next_appointmet_time').val()
	//console.log("next_time "+next_time)
	$.when(fetch_all_lab_details(url_val)).then(()=>{
		 re_suchedule(next_date,next_time);
	})
	

})
function re_suchedule(next_date,next_time){
		$.ajax({
		url:"action/appointment/re_schedule_appointment.php",
		type:"POST",
		...
		success:function(appointment_result){
			...
			if(appointment_result == 1){
					$(this).text('Success')
					$(this).css('background-color','blue')
				setTimeout(function(){
					$(this).text('Submit')
					$(this).css('background-color','#557bfe')
					$('.datePopup').fadeOut();
					$('.shimmer').fadeOut();
					
					window.location.href="scheduled-appointments.php"
				},1000)
...
setInterval(fetch_all_patient_start_meeting, 12000);
fetch_all_patient_start_meeting()
function fetch_all_patient_start_meeting(){
	$.ajax({
		url:"action/appointment/fetch_all_patient_start_meeting.php",
		type:"POST",
		data:{url_val:url_val},
		success:function(meeting_result){
			let meeting_result_json = jQuery.parseJSON(meeting_result)
			$('.nextMeetingList').empty();
			if(meeting_result_json.length){
				for(let x = 0;x<meeting_result_json.length;x++){
			 $('.notificationSound').get(0).play();
					console.log(meeting_result_json[x]['name'])
				$('.nextMeetingList').append(`<div class="nextMeetingListBox" style="display:flex">
						<div class="nextMeetingListBoxName">
							<p>${meeting_result_json[x]['name']}</p>
						</div>
						<a href="online-consultation.php?id=${meeting_result_json[x]['id']}"  class="consultNextMeetingListBox">
							<i class="uil uil-check"></i>
						</a>
						<div class="closeNextMeetingListBox" data-id=${meeting_result_json[x]['id']}>
...
$('.consultNextMeetingListBox').click(function(){
	$(this).parent().fadeOut();
})
$('body').delegate('.closeNextMeetingListBox','click',function(){
	let data_id = $(this).attr('data-id')
	let that = $(this)
	$.ajax({
		url:"action/appointment/close_appointment.php",
		type:"POST",
		data:{data_id:data_id},
		success:function(){
			that.parent().fadeOut();
