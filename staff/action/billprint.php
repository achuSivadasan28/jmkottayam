...
$x = 0;
//$id=$_POST['id'];
$customer = $obj1->selectData("id,tax_invoice_year,tax_invoice,tax_compained_val,invoice_no,customer_name,staff_id,phone,created_date,total_price,total_discount,total_amonut,tax_apply,total_tax_amt,time","tbl_customer","where id=$customer_id  ORDER BY id DESC");
if(mysqli_num_rows($customer)>0){
    while($customer_row = mysqli_fetch_array($customer)){
		...
		$total_price_q = 0;
		$total_tax = 0;
        $fetch_data = $obj1->selectData("id,customer_id,category_name,product_name,price,no_quantity,gross,total_price,discount,disc,hsn_number,batch_name,expiry_date,tax_in_per,tax_data","tbl_productdetails","where customer_id=$id");
