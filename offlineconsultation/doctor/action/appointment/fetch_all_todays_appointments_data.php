...
if($check_security == 1){
	...
	if(mysqli_num_rows($select_todays_appointment)>0){
		...
		while($select_todays_appointment_row = mysqli_fetch_array($select_todays_appointment)){
			...
			$select_bmi_history = $obj_branch->selectData("height,weight,appointment_date,blood_pressure","tbl_appointment","where patient_id=$patient_id and branch_id = $branch ORDER BY id DESC");
