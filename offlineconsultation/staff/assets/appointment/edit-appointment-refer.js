...
$('.confirmconfirmAlert').click(function(){
	...
	$.ajax({
		url:"action/appointment/update_appointment-refer.php",
		type:"POST",
		data:{name:name,
			  number:number,
			  address:address,
		...
		success:function(appointment_result){
			...
			if(result_data_json[0]['status'] == 0){
				...
				setTimeout(function () {
					...
					let current_page = current_url[current_url.length - 1].split('?')[0]
					if(current_page == "edit-appointment-details.php"){
						window.location.href="appointments.php";
					}else{
						window.location.href="refer-appointments.php";
					}
...
$('#appointment_btn').click(function(e){
	...
	if(error_msg == 0){
		if($('#appointment').val() != 0){
			if(total_amount == $('#appointment').val()){
				$('#amt_error_msg').text('');
				$('.confirmAlert').css('display','block')
				$('.shimmer').fadeIn();

			}else{
				$('#amt_error_msg').text('* amount mismatch');
...
function fetch_appointment_fee(){
	$.ajax({
		type:'post',
		url:"action/appointment/fetch_appointment_fee_refer.php",
		data:{id:url_val},
		success:function(appointment_fee){
			...
			if(appointment_fee_json[0]['status'] == 1){
				...
				$('#no_appointment_fee').text(appointment_fee_json[0]['appointment_fee_msg'])
	...
	$.ajax({
		...
		success:function(result){
			...
			if(result_json_data.length>0){
				if(result_json_data[0]['payment_amnt'] != 0){
					$('#paymentMode').empty();
				}
