...
if($check_security == 1){
	...
	if(sizeof($food_avoided_plan)!=0){
		for($x1=0;$x1<sizeof($food_avoided_plan);$x1++){
			$check_food_data = $obj->selectData("id","tbl_foods_avoid","where appointment_id=$patient_id and foods_to_be_avoided='$food_avoided_plan[$x1]' and added_date='$days' and status=1");
		...
		for($x2=0;$x2<sizeof($diet_plan);$x2++){
			$check_dite_plan = $obj->selectData("id","tbl_diet_followed","where appointment_id=$patient_id and diet='$diet_plan[$x2]' and added_date='$days' and status=1");
