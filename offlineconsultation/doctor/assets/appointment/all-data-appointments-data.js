...
function fetch_all_appointments(){
	$.ajax({
		url:"action/appointment/fetch_all_online_appointment_count.php",
		type:"POST",
...
$.ajax({
	url:"action/appointment/fetch_all_online_appointment.php",
	type:"POST",
	data:{search_val:search_val,
			...
			if(result_data_json[0]['data_status'] == 1){
				...
				for(let x=0;x<result_len;x++){
					...
					let appoint_status = 'Pending';
					 print_data = '';
					let eye_btn = '';
					eye_btn = `<div class="tableBtnArea"><a href="view-appointment-details.php?id=${result_data_json[x]['app_id']}" class="tableViewBtn" title="View"><i class="uil uil-eye"></i>
                                                            </a></div>`;
					if(result_data_json[x]['appointment_status'] == 2){
						print_data = `<div class="tableBtnArea"><a  href="#" class="currentConsultingheadPrintBtn tablePrintBtn" data-id=${result_data_json[x]['app_id']}><i class="uil uil-print"></i></a></div>`;
						appoint_status = 'Counsulted';
						eye_btn = `<div class="tableBtnArea"><a href="view-appointment-details.php?id=${result_data_json[x]['app_id']}" class="tableViewBtn" title="View"><i class="uil uil-eye"></i>
                                                            </a></div>`
					}else{
						if(result_data_json[x]['appointment_taken_date'] < result_data_json[x]['current_date']){
							appoint_status = "Appointment Missed";
						}
						eye_btn = `<div class="tableBtnArea"><a href="view-appointment-details.php?id=${result_data_json[x]['app_id']}" class="tableViewBtn" title="View"><i class="uil uil-eye"></i>
                                                            </a></div>`
					}
				...
				$('.tableWraper table tbody').append(`<tr>
													...
													<td data-label="bookig id">
                                                        <p>${result_data_json[x]['booking_id']}</p>
                                                    </td>
                                                    <td data-label="Phone">
                                                    ...
                                                    <td data-label="Date">
                                                        <p>${result_data_json[x]['appointment_taken_date']}</p>
                                                    </td>
<td data-label="Date">
                                                        <p>${result_data_json[x]['online_confirm_date']}</p>
                                                    </td>
