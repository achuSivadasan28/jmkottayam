			...
			for(let x=0;x<result_data_json.length;x++){
				...
				if(result_data_json[x]['active'] == 1){
					...
					$('.currentConsultingheadPrintBtn').attr('data-id',result_data_json[x]['id'])
						...
						fetchLab_data(result_data_json[x]['id'])
						fetch_added_lab(result_data_json[x]['id'])
			...
			$.ajax({
				...
				success:function(appointment_result){
					...
					if(appointment_result_json[x]['present_Illness'] != ''){
						...
						fetchLab_data(appointment_result_json[x]['id'])
						fetch_added_lab(appointment_result_json[x]['id'])
					$('.consultingWindowTable table tbody').empty()
					$('.currentConsulting').fadeIn();
...
fetch("https://pala.jmwell.in/api/fetchAll_medicine.php",{
	method:"POST",
...
$('body').delegate('.medicine_details','change',function(){
	...
	fetch("https://pala.jmwell.in/api/check_medicine_stock.php",{
			method:"POST",
    		header:{'Content-Type':'application/json'},
...
function fetch_added_lab(url_val){
$.ajax({
	...
	success:function(appointment_result){
		...
		let appointment_result_json = jQuery.parseJSON(appointment_result)
		console.log(appointment_result_json)
		if(appointment_result_json.length !=0){
		for(let x=0;x<appointment_result_json.length;x++){
			let template_data = `<div class="labTestHistorySectionBox">
									<div class="labTestHistoryDate">${appointment_result_json[x]['added_date']}</div>
									<div class="labTestHistoryReport">`;
			let lab_data = appointment_result_json[x]['lab']
			console.log(lab_data.length)
			for(let x1=0;x1<lab_data.length;x1++){
				
				template_data += `<div class="labTestHistoryReportBox">
											<p>${lab_data[x1]['test_name']}</p>
											<a href="assets/fileupload/${lab_data[x1]['lab_report_file']}" class="labTestHistoryReportFileBtn spotlight">
			...
			$('.labTestHistorySection').append(`<div class="elseDesign" style="display:flex">
...
function fetchLab_data(url_val){
$.ajax({
			...
			for(let x=0;x<lab_result_json.length;x++){
				$('.commentsPopupPreviousMain').append(`<div class="commentsPopupPrevious">
								...
								<div class="labTestList">
									<a href="add-lab-test.php?id=${lab_result_json[x]['id']}" class="labTestListBox">${lab_result_json[x]['test_name']}</a>
								</div>
							</div>`)
			}
		}else{
