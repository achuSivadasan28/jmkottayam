...
$days=date('d-m-Y');
$invoice_id = $_POST['search_val'];
$response_arr = array();
  $select_customer_details = $obj->selectData("id,customer_name,phone,place,total_price,total_discount,total_amonut","tbl_customer","where invoice_no like '%$invoice_id%'");
if(mysqli_num_rows($select_customer_details)>0){
	while($select_customer_details_row = mysqli_fetch_array($select_customer_details)){
		...
		$select_all_products = $obj->selectData("id,product_name,discount,no_quantity,price,total_price,product_id,no_pills,pills_id,tax_in_per,tax_data,expiry_date","tbl_productdetails","where customer_id=$id and status !=0");
