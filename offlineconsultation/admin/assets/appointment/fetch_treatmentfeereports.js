...
fetch('action/appointment/fetch_all_treatmentfeereport.php?startdate=' + startDate + '&enddate=' + endDate + '&search=' + searchval_name +'&limit_range='+limit_range)
    ...
    $('.elseDesign').css({
      display: 'flex'
	   ...
	   $('.pagination').show();
	  let gpay = (data[0]['gpay'] == undefined || data[0]['gpay'] == null) ? 0 : data[0]['gpay']
	  let cash = (data[0]['cash'] == undefined || data[0]['cash'] == null) ? 0 : data[0]['cash']
	  let card = (data[0]['card'] == undefined || data[0]['card'] == null) ? 0 : data[0]['card']
	  ...
	  $('#treatment').text('₹ '+ treatment_count)
	  let total = parseInt(gpay) + parseInt(cash) + parseInt(card)
	  
	  $('#tmt_amt_data').text('₹ '+ total)
 ...
 <tr>
                                                    <td data-label="Sl No">
                                                      ...
                                                      <td data-label="Name">
                                                        <p>${data[x]['invoice_num_com']}</p>
                                                    </td>
                                                    <td data-label="Name">
                                                        <p>${data[x]['patient_name']}</p>
                                                    </td>
                                                   <td data-label="Date">
                                                        <p>${data[x]['added_date']}</p>
                                                    </td>
                                                <td data-label="Date">
                                                        <p>${data[x]['tot_amt']}</p>
                                                    </td>
													<td data-label="payment mode data">
                                                        <p>${data[x]['payment_mode']}</p>
                                                    </td>
...
$('.pageLoader').css('display','none')
	...
	$('.export_reports').attr('href', 'action/appointment/export_all_treatmentfee.php?startdate=' + startDate + '&enddate=' + endDate  + '&search=' + searchval_name)
}
...
function fetch_fee_reports(){
	...
	return $.ajax({
		url:"action/appointment/fetch_treatmentfee_count.php",
		type:"POST",
...
function createPagination(totalPages, page){
  ...
  $('.pagination ul').html(liTag) //add li tag inside ul tag
  console.log(liTag)
