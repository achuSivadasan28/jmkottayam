...
while($data=mysqli_fetch_array($selectData))
{
	...
	$select_product_details = $obj->selectData("id,product_id,no_of_pills,price,quantity,tax_data,expiry_date","tbl_medicine_details","where 		no_of_pills>=$quantity_need and product_id=$pid and status != 0 ORDER by no_of_pills ASC limit 1");
...
for($y1 = 0; $y1<sizeof($dataArray);$y1++){
	...
	$select_tax_ratio =  $obj->selectData("tax_data","tbl_medicine_details","where id=$pill_id and status!=0");
