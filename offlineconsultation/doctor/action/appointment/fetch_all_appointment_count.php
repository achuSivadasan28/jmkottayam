...
if($check_security == 1){
	...
	if($login_id == 86){
		...
		$doctor_where = "(tbl_appointment.doctor_id='$login_id' or tbl_appointment.consulted_by='$login_id')";
	}
	...
	$select_todays_appointment = $obj->selectData("count(tbl_appointment.id) as id","tbl_appointment inner join tbl_patient on tbl_appointment.patient_id=tbl_patient.id","where $doctor_where $where_search_data $where_date_clause and tbl_appointment.status!=0");
