...
$fileName = "Johnmarians_Top_Selling_report_trvl" . date('Y-m-d') . ".xls"; 
...
$export_arr = array();
$x = 0;
$fetch_data = $obj1->selectData("tbl_product.id,tbl_product.product_name,tbl_product.category_id,tbl_medicine_details.quantity","tbl_product inner join tbl_medicine_details on tbl_product.id=tbl_medicine_details.product_id","where tbl_product.status!=0 ORDER BY tbl_product.id DESC");
	...
	while($fetch_data_row = mysqli_fetch_array($fetch_data)){
		...
		if(mysqli_num_rows($category)>0){
			while($category_row = mysqli_fetch_array($category)){
				...
				$p_details=$obj1->selectData("tbl_productdetails.no_quantity,tbl_productdetails.product_id","tbl_productdetails INNER JOIN tbl_customer on tbl_productdetails.customer_id=tbl_customer.id","where tbl_productdetails.product_id=$product_id");
				//print_r($obj1);exit();
		...
		if($analysis>50){
			...
			$lineData = array($y,$product_name,$category_name,$t_qunatity,$product_quantity,$left,$analysis); 
