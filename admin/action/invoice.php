...
if(mysqli_num_rows($customer)>0){
    while($customer_row = mysqli_fetch_array($customer)){
		...
		$invoice_date = $customer_row['created_date'];
		$dates = new DateTime($invoice_date);
		$invoice_month = $dates->format('m');
        ...
        $response_arr[$x]['discount'] = $customer_row['total_discount'];
		 $response_arr[$x]['current_month'] = $currentMonth;
		 $response_arr[$x]['invoice_month'] = $invoice_month;
...
$total_amt_data = 0;
$total_tax_amt = 0;
	$customer_sum = $obj1->selectData("id","tbl_customer","where status!=0 $search_where $where_staff_clause $where_search_date ORDER BY id DESC");
	if(mysqli_num_rows($customer_sum)>0){
		while($customer_sum_row = mysqli_fetch_array($customer_sum)){
		$c_id = $customer_sum_row['id'];
		$fetch_data_sum = $obj1->selectData("sum(total_price) as total_price,sum(tax_data) as tax_data","tbl_productdetails","where customer_id=$c_id and status!=0");
		$fetch_data_sum_row = mysqli_fetch_array($fetch_data_sum);
		if($fetch_data_sum_row['total_price'] != null){
			$total_amt_data +=  $fetch_data_sum_row['total_price'];
		}
		if($fetch_data_sum_row['tax_data'] != null){
			$total_tax_amt += $fetch_data_sum_row['tax_data'];
		}
		}
		
	}
		
	$total_actual_amt = $total_amt_data-$total_tax_amt;
