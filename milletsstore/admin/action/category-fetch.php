...
$fetch_data = $obj1->selectData("id,category_name,address,gstNum,phnNum","tbl_category","where status!=0 $search_where ORDER BY id DESC limit $limit");
if(mysqli_num_rows($fetch_data)>0){
 ...
 while($fetch_data_row = mysqli_fetch_array($fetch_data)){
	...
	$response_arr[$x]['address'] = $fetch_data_row['address'] ;
	$response_arr[$x]['gstNum'] = $fetch_data_row['gstNum'] ;
	$response_arr[$x]['phnNum'] = $fetch_data_row['phnNum'] ;
