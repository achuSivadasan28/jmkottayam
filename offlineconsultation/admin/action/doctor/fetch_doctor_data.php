...
if($check_security == 1){
	...
	$select_all_branch = $obj->selectData("tbl_doctor.id,doctor_name,phone_no,email,department_id,branch_id,login_id,tbl_login.role","tbl_doctor join tbl_login on tbl_doctor.login_id = tbl_login.id","where tbl_doctor.id=$doctor_id and  tbl_doctor.status!=0 ORDER BY id DESC");
	if(mysqli_num_rows($select_all_branch)>0){
		...
		while($select_all_branch_row = mysqli_fetch_array($select_all_branch)){
			...
			$response_arr[$x]['role'] = $select_all_branch_row['role'];
