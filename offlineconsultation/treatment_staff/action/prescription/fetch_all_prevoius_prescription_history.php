...
if($check_security == 1){
	...
	if(mysqli_num_rows($select_all_prescription_data)>0){
		while($select_all_prescription_data_row = mysqli_fetch_array($select_all_prescription_data)){
			...
			$check_prescriptions_today = $obj->selectData("id,prescriptions_added_date,prescriptions_added_time","tbl_prescriptions","where prescriptions_added_date='$days' and status=1 and patient_id=$patient_data ORDER BY id DESC");
			...
			$select_all_prescription_id = $obj->selectData("id,prescriptions_added_date,prescriptions_added_time","tbl_prescriptions","where status!=0 and patient_id=$patient_data ORDER BY id DESC limit 1");
