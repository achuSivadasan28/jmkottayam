...
$fetch_patient = $obj_branch->selectData("id,name,phone,age,place,unique_id","tbl_patient","where id='$url_val'");
...
$unique_id = $patient_details['unique_id'];
$obj_common = new query_common();
$select_url = $obj_common->selectData("image_uploadurl,image","tbl_patient","where unique_id = '$unique_id' limit 1");
if($select_url_row = mysqli_fetch_assoc($select_url)){
	$response_arr[0]['image_url'] = $select_url_row['image_uploadurl'];
	$response_arr[0]['image'] = $patient_details['image'];

}

$select_bmi_details = $obj_branch->selectData("weight,height,diet_no_of_days,dite_remark","tbl_appointment","where patient_id=$url_val and branch_id = $branch order by id desc limit 1");
	...
	$fetch_count = $obj_branch->selectData("count(id) as id","tbl_appointment","where patient_id ='$url_val' and appointment_status ='2'");
	$total_count= mysqli_fetch_assoc($fetch_count);
$total_number_visit=$total_count['id'];

	 $select_todays_appointment = $obj_branch->selectData("tbl_appointment.id,tbl_appointment.appointment_status,tbl_appointment.branch_id,tbl_appointment.real_branch_id,tbl_appointment.consulted_by,tbl_appointment.doctor_id,tbl_appointment.appointment_taken_type,tbl_appointment.appointment_date,tbl_appointment.patient_id,tbl_appointment.appointment_number,tbl_appointment.height,tbl_appointment.weight,tbl_appointment.blood_pressure,tbl_appointment.allergies_if_any,tbl_appointment.current_medication,tbl_appointment.present_Illness,tbl_appointment.any_surgeries,tbl_appointment.any_metal_Implantation,tbl_patient.unique_id,tbl_patient.name,tbl_patient.phone,tbl_patient.address,tbl_patient.place,tbl_patient.age,tbl_patient.gender,tbl_patient.id as patient_id,tbl_appointment.cross_branch_status,tbl_appointment.cross_branch_id","tbl_appointment inner join tbl_patient on tbl_appointment.patient_id=tbl_patient.id ","where (tbl_appointment.appointment_status=2 or tbl_appointment.online_conformation_status = 1)and tbl_appointment.patient_id=$url_val and tbl_appointment.branch_id = $branch order by tbl_appointment.id desc ");
//echo $select_todays_appointment;exit();
	if(mysqli_num_rows($select_todays_appointment)>0){
		...
		while($select_todays_appointment_row = mysqli_fetch_array($select_todays_appointment)){
			$appointment_type = $select_todays_appointment_row['appointment_taken_type'];
			if($select_todays_appointment_row['consulted_by'] != 0){
				$doctor_id =  $select_todays_appointment_row['consulted_by'];
			}else{
				$doctor_id =  $select_todays_appointment_row['doctor_id'];
			}
		//$doctor_id=$select_todays_appointment_row['doctor_id'];
			$appointment_id=$select_todays_appointment_row['id'];
			if($select_todays_appointment_row['cross_branch_status'] == 1){
				$branch = $select_todays_appointment_row['cross_branch_id'];
			
			}else{
				$branch = $select_todays_appointment_row['real_branch_id'];
			}
			
			$obj_branch = new query_branch();
				 $fetch_doctor = $obj_branch->selectData("doctor_name","tbl_doctor","where login_id= '$doctor_id'");
	$doc= mysqli_fetch_assoc($fetch_doctor);
$doctor_name=$doc['doctor_name'];
			
				
			$app_id =  $select_todays_appointment_row['id'];
			...
			if($appointment_type == "Online"){
				$select_appointment_times = $obj_branch->selectData("consulted_date,appointment_id","tbl_online_appointment_sitting_history","where appointment_id = $app_id");
				
				if(mysqli_num_rows($select_appointment_times)){
					$y = 0;
					while($select_appointment_times_rows = mysqli_fetch_assoc($select_appointment_times)){
						$response_arr[$x]['prescription_date'][$y]['date'] = $select_appointment_times_rows['consulted_date'];
						$y++;
					}
				}
			}
