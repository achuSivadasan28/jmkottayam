...
if($check_security == 1){
	$select_all_branch = $obj->selectData("id,staff_name,staff_phone,staff_email,branch_id,login_id","tbl_staff","where id=$staff_id and  status!=0 ORDER BY id DESC");
	if(mysqli_num_rows($select_all_branch)>0){
		...
		while($select_all_branch_row = mysqli_fetch_array($select_all_branch)){
			...
			$select_role = $obj->selectData("role","tbl_login","where id=$login_id");
			if(mysqli_num_rows($select_role)>0){
				while($select_role_row = mysqli_fetch_array($select_role)){
					$response_arr[$x]['role'] = $select_role_row['role'];
				}
			}
