...
if($check_security == 1){
	...
	$branch = $_POST['g_branch_id'];
	$select_actual_appointment_data = $obj->selectData("id,real_branch_id,branch_id","tbl_appointment","where id=$url_val");
	if(mysqli_num_rows($select_actual_appointment_data)>0){
		$select_actual_appointment_data_row = mysqli_fetch_array($select_actual_appointment_data);
		if($select_actual_appointment_data_row['real_branch_id'] == $select_actual_appointment_data_row['branch_id']){
	$select_actual_appointment = $obj->selectData("id","tbl_appointment","where id=$url_val");
	if(mysqli_num_rows($select_actual_appointment)>0){
		$select_actual_appointment_row = mysqli_fetch_array($select_actual_appointment);
		$url_val = $select_actual_appointment_row['id'];
	}
	...
	$select_actual_appointment = $obj_branch->selectData("id","tbl_appointment","where cross_appointment_id=$url_val");
	if(mysqli_num_rows($select_actual_appointment)>0){
		$select_actual_appointment_row = mysqli_fetch_array($select_actual_appointment);
		$url_val = $select_actual_appointment_row['id'];
	}
	$select_dite_plan = $obj_branch->selectData("diet","tbl_diet_followed","where appointment_id=$url_val and status!=0");
	if(mysqli_num_rows($select_dite_plan)>0){
		$x = 0;
		$response_arr[0]['dite'][0]['dite_data_status'] = 1;
		while($select_dite_plan_row = mysqli_fetch_array($select_dite_plan)){
	...
	$select_food_data = $obj_branch->selectData("foods_to_be_avoided","tbl_foods_avoid","where appointment_id=$url_val and status!=0");
	if(mysqli_num_rows($select_food_data)>0){
		$x1 = 0;
		$response_arr[0]['food'][0]['food_data_status'] = 1;
		while($select_food_data_row = mysqli_fetch_array($select_food_data)){
	...
	$select_dite_details = $obj_branch->selectData("diet_no_of_days,dite_remark,main_remark","tbl_appointment","where id=$url_val and status!=0");
	if(mysqli_num_rows($select_dite_details)>0){
		while($select_dite_details_row = mysqli_fetch_array($select_dite_details)){
			$response_arr[0]['diet_no_of_days'] = $select_dite_details_row['diet_no_of_days'];
			$response_arr[0]['dite_remark'] = $select_dite_details_row['dite_remark'];
